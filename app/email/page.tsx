  ...
  async function run() {
    ...
    try {
      const res = await fetch("/api/ai-tool", {
        ...
        body: JSON.stringify({ tool: "email", input, options: {} }),
      });
      const json = await res.json();
      setResult(json.result || json.error || "エラーが発生しました");
      if (json.result) recordToolUse("/email", "メール文面作成");
    } catch {
    ...
    <div style={{ minHeight: "100vh", background: "var(--bg)" }}>
      ...
      <div style={{ maxWidth: 800, margin: "0 auto", padding: "2.5rem 1.5rem" }}>
        <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: "2rem" }}>
          <div style={{ width: 48, height: 48, borderRadius: 12, background: "#f9731622", display: "flex", alignItems: "center", justifyContent: "center" }}>
            <IconEmail size={24} style={{ color: "#f97316" }} />
          </div>
        ...
        <div style={{ background: "var(--bg2)", border: "1px solid var(--border)", borderRadius: 14, padding: "1.5rem", marginTop: "1.5rem" }}>
          ...
          <textarea
            ...
            placeholder="状況・目的・相手を簡単に入力してください..." rows={7}
          ...
          <button onClick={run} disabled={loading || !input.trim()}
            style={{ width: "100%", background: input.trim() && !loading ? "#f97316" : "#333", color: "#fff", border: "none", borderRadius: 10, padding: 14, fontSize: 16, fontWeight: 700, cursor: "pointer", marginTop: 12, display: "flex", alignItems: "center", justifyContent: "center", gap: 8 }}>
